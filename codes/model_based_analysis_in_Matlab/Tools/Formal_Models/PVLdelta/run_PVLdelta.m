function [A R] = run_PVLdelta(A, data)
...
switch A.fit.priors.type
    ...
    case 'informed' % use priors derived from firstpass
        ...
        priors.muPhi = -1.5093;
        priors.SigmaPhi =  0.8383^2;
        priors.muTheta = [-0.3423   -1.8023   -0.6182];
        priors.SigmaTheta = [1.24481344017957,0,0;0,1.36854902939521,0;0,0,1.77908041228778;].^2;
        priors.muX0(1:4,1) = [0 0 0 0];
...
% inF = evolution
options.inF.param_name = {'Sensitivity (value)', 'Loss aversion (value)', 'Update (value)'};
