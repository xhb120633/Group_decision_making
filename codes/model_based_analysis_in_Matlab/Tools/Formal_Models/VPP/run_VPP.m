...
switch A.fit.priors.type
    ...
    case 'informed' % use priors derived from first pass
        ...
        priors.muPhi = [-1.1891   -0.5810];
        priors.SigmaPhi = [0.691711421317257,0;0,0.459444882057106;].^2;
        priors.muTheta = [-0.3217   -1.5005   -0.8164   -0.1280    0.2059   -0.0458];
        priors.SigmaTheta = [0.996574201664647,0,0,0,0,0;0,1.27636688659613,0,0,0,0;0,0,1.51457538407198,0,0,0;0,0,0,1.50164394799350,0,0;0,0,0,0,0.548514858529676,0;0,0,0,0,0,0.647715904991372;].^2;
        priors.muX0(1:8,1) = [0 0 0 0 0 0 0 0];
        ...
        Tsig0to5 = @(x) sig(x)*5;
        TsigMin2to2 = @(x) sig(x)*4-2;
        Tsigmin1plus1 = @(x) sig(x)*2-1;

    case 'shrinkage'

        priors = []; % default settings of the VBA toolbox
        priors.muX0(1:8,1) = [0 0 0 0 0 0 0 0];%hs.initval;
...
% inF = evolution
options.inF.param_name = {'Sensivity (value)', 'Loss aversion (value)', 'Updating (value)', '(inverse) Decay', 'Perseveration gain', 'Perserveration loss'};
options.inF.param_transform = {Tsig, Tsig0to5, Tsig, Tsig, Tsigmin1plus1,Tsigmin1plus1};
% inF = evolution (learning)
options.inG.param_name = {'Expectancy weight', 'Consistency'};
options.inG.param_transform = {Tsig Tsig0to5};
