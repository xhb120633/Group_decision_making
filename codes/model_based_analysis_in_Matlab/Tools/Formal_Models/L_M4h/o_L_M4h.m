...
%source 1
 function g1=g_source1(x,P,u,in)
        for pp =1:length(P)  
            ...
            consistency = 3^P(1)-1;
            g1 = exp((x(1:4)+x(5:8)+u(13:16).^P(2))*consistency)/sum(exp((x(1:4)+x(5:8)+u(13:16).^P(2))*consistency));
...
function g2=g_source2(x,P,u,in)
        for pp =1:length(P)  
            ...
            g2 =sub_softmax((P(2).^cons).*(x(9:12)+x(13:16)+u(8:11).^P(2)), consistency);
