...
Figure_4a <-
  ggplot(data = mean_fitting_acc, aes(x = Discuss,y=Mean,group=Leader,fill=Leader)) +  
  scale_fill_discrete(labels=c("No Leader Group","Leader Group - Member","Leader Group - Leader"))+
  geom_col(width =0.35, position = position_dodge(width = 0.4),alpha=0.6)+
  geom_errorbar(position = position_dodge(width = 0.4),aes(x=Discuss,ymin=Mean-SE,ymax=Mean+SE,group=Leader),size=1,alpha=1,width=0.06)+
  geom_point(data=fitting_acc,aes(x=Discuss,y=Accuracy,group=Leader,color=Leader),position = position_dodge(width = 0.4),alpha=0.8)+guides(color=FALSE)+
  scale_x_discrete("Discussion", labels = c("0" = "No Discussion", "1" = "Discussion")) +
  scale_y_continuous(expand = c(0,0))+
  labs(y = "Fitting Accuracy") +
  pic_theme+
...
p3<-ggplot()+
  ...
  geom_point(data=individual_simulation_acc,aes(x=discuss_list,y=accuracy,color=leader_power_list),alpha=0.8,size=4,position=position_jitterdodge(jitter.width=0.05,dodge.width=0.8))+guides(color=FALSE)+
...
Figure_4c <-
  ggplot(data = mean_g_p, aes(x = Discuss,y=Mean_Accuracy,group=Leader,fill=Leader)) +  
  geom_col(width =0.35, position = position_dodge(width = 0.4),alpha=0.75)+
  geom_point(data=g_p,aes(x=Discuss,y=Group_Accuracy,group=Leader,color=Leader),position = position_jitterdodge(dodge.width = 0.4,jitter.width=0.05),alpha=0.8,size=4)+guides(color=FALSE)+
  geom_errorbar(position = position_dodge(width = 0.4),aes(x=Discuss,ymin=Mean_Accuracy-sd,ymax=Mean_Accuracy+sd,group=Leader),size=1,alpha=1,width=0.06)+
  scale_x_discrete("Discussion", labels = c("0" = "No Discussion", "1" = "Discussion")) +
  scale_y_continuous(expand = c(0,0))+
  scale_color_manual(values=c('#4daf4a','#e41a1c'),labels=c('No Leader Group','Leader'))+
  scale_fill_manual(values=c('#4daf4a','#e41a1c'),labels=c('No Leader Group','Leader Group'))+
  labs(y = "Simulation Accuracy-Group Decision") +
  pic_theme+
  theme(legend.title =element_blank(),
        legend.position = c(0.5, 0.90))
