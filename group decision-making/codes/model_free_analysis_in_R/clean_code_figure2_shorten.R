...
Figure_2a <-
  ...
  scale_fill_manual(values=c('#0943f0','#4daf4a','#e41a1c'),labels=c("Individual","No Leader","Leader"))+
  ...
  scale_color_manual(values=c('#0943f0','#4daf4a','#e41a1c'),labels=c("Individual","No Leader","Leader"))+
...
Figure2c<-
  ...
  scale_fill_manual(values=c('#0943f0','#4daf4a','#984ea3','#e41a1c'),labels=c('Individual',"No Leader","Non-Leading Member","Leader"))+
  geom_col(width =c(0.4/3,rep(0.4,6)),position = position_dodge(width = 0.55),alpha=1,size=1)+
  ...
  geom_errorbar(position = position_dodge(width = 0.55),aes(x=discuss,ymin=mean-se,ymax=mean+se,group=leader),size=1.2,alpha=1,width =c(0.2/3,rep(0.2,6)))+
  ...
  scale_y_continuous(expand = c(0,0))+
  scale_color_manual(values=c('#0943f0','#4daf4a','#984ea3','#e41a1c'))+
...
ggplot(data=tmp_data,aes(color=reward,group=reward))+
...
#tmp_data<-aggregate(.~leader+discuss+N+reward+tmp_id,FUN = 'mean',data = i_data_new)
tmp_data<-subset(i_data_new,leader==1&discuss==1)
tmp_data$reward<-as.factor(tmp_data$reward-0.5)
...
#segment data
  ...
  tmp_data<-subset(plot_raw_data2, leader==1&discuss==1)
